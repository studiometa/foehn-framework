...
- The cache is served by whichever reader the site can offer, and all of them compute the same filename by construction. `wp foehn cache:config --server=nginx|apache` generates a server snippet from the loaded configuration — so the cookies, the ignored query args and the cache path in it cannot drift from the PHP that wrote the files — and the generated snippet composes with an existing `location /` rather than replacing it. Where there is no server access, an `advanced-cache.php` drop-in written by the installer serves the same files before WordPress loads, and is the only reader that can enforce a TTL per request or answer with a `304`
