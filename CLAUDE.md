...
- `packages/starter/composer.json` and `packages/demo/composer.json` pin `studiometa/foehn` and `studiometa/foehn-installer` to an **exact** version, so `composer create-project` installs a combination somebody has run. **Every** release commit moves those four pins to the version being tagged. `0.6.0` shipped a starter requiring `^0.5` — a caret on a `0.x` pins the minor, so create-project kept building `0.5.10` projects. The release workflow now resolves each pin against the tag and refuses a tag its own starter would not install
