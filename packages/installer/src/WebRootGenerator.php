...
final class WebRootGenerator
{
     ...
     *
     * The generated wp-config.php refuses to serve a production request without them,
     * so an install that did not produce them would leave a site that cannot start.
     * They are never rewritten: doing so on every `composer install` would log every
     * user out on every deploy.
     *
     * This repeats what `Studiometa\Foehn\Security\Salts` does, because a Composer
     * plugin cannot rely on the project's autoloader being available to it. The file
     * format is the contract, and `wp foehn salts:generate` writes the same one.
     */
    private function generateSalts(): void
    {
        $path = $this->projectRoot . '/' . $this->config->configDir . '/wordpress-salts.config.php';
        ...
        $lines = [
            ...
            '',
            '/**',
            ' * WordPress security keys — generated by foehn-installer, and secret.',
            ' *',
        ...
        foreach ([
            ...
            $lines[] = sprintf("define('%s', '%s');", $name, addslashes(base64_encode(random_bytes(48))));
        }

        $this->writeRawFile($path, implode("\n", $lines) . "\n");

        // Secrets are readable by their owner alone.
        chmod($path, 0o600);
